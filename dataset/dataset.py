...
class cubic_sequence_data(data.Dataset):
    def __init__(self, pattern='train', num_fold=0, num_class=3):

        raw_dataset = np.load(opt.test_dataset_root) if pattern == 'test' else np.load(opt.train_dataset_root)
        ...
        if num_class == 2: self.label_array[self.label_array == 2] = 1
